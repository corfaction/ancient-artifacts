...
public final class ArchaeologicalTableScreen extends AbstractContainerScreen<ArchaeologicalTableMenu> {
    ...
    private void renderArtifact(
        ...
        for (int pixelY = 0; pixelY < TEXTURE_SIZE; pixelY++) {
            for (int pixelX = 0; pixelX < TEXTURE_SIZE; pixelX++) {
                if (!mask.isPixelPresent(pixelX, pixelY) ||
    ...
    private int[] getArtifactPixel(double mouseX, double mouseY) {
        int guiX = (width - imageWidth) / 2;
        int guiY = (height - imageHeight) / 2;
        int localX = (int) mouseX - guiX - ARTIFACT_X;
        int localY = (int) mouseY - guiY - ARTIFACT_Y;
        ...
        return new int[]{localX / PIXEL_SIZE, localY / PIXEL_SIZE};
    }
    ...
    private boolean tryCleanPixel(double mouseX, double mouseY) {
        ...
        if (mask == null || !mask.isPixelPresent(pixel[0], pixel[1])) {
            return false;
        }
